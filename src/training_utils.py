...
def train(run, graph, ops, X_train, Y_train, X_val, Y_val, batch_size,
          num_epochs1, num_epochs2, steps_per_epoch, validation_steps, patience=5,
    ...
    Arguments:
        ...
        num_epochs1: An integer.
        num_epochs2: An integer.
    ...
    Returns:
        A list of tuples containing train and test logloss/accuracy.
    ...
    # get graph's ops
    data_init_op, predictions_op, log_loss_op, train_log_loss_op,\
        optimize1_op, optimize2_op,\
        init_op, saver_op, accuracy_op, summaries_op = ops
    ...
    if verbose:
        print('first stage begins')

    losses1 = _training_loop(
        sess, writer,
        optimize1_op, summaries_op, log_loss_op, train_log_loss_op, accuracy_op,
        initial_epoch, num_epochs1, steps_per_epoch, validation_steps,
        verbose, patience
    )
    ...
    losses2 = _training_loop(
        sess, writer,
        optimize2_op, summaries_op, log_loss_op, train_log_loss_op, accuracy_op,
        num_epochs1 + 1, num_epochs2, steps_per_epoch, validation_steps,
        verbose, patience
    )
...
def _training_loop(sess, writer,
                   optimize_op, summaries_op,
                   log_loss_op, train_log_loss_op, accuracy_op,
                   initial_epoch, num_epochs, steps_per_epoch, validation_steps,
                   verbose, patience):
    ...
    for epoch in training_epochs:
        ...
        running_loss, running_train_loss, running_accuracy = 0.0, 0.0, 0.0
        ...
        _, batch_loss, batch_train_loss, batch_accuracy, summary = sess.run(
            [optimize_op, log_loss_op, train_log_loss_op, accuracy_op, summaries_op],
        ...
        for step in range(1, steps_per_epoch):
            ...
            _, batch_loss, batch_train_loss, batch_accuracy = sess.run(
                [optimize_op, log_loss_op, train_log_loss_op, accuracy_op]
            )
        ...
        test_loss, test_train_loss, test_accuracy = _evaluate(
            sess, validation_steps, log_loss_op, train_log_loss_op, accuracy_op
        )
        train_loss = running_loss/steps_per_epoch
        train_train_loss = running_train_loss/steps_per_epoch
        train_accuracy = running_accuracy/steps_per_epoch

        if verbose:
            print('  {0}      {1:.3f} {2:.3f}   {3:.3f} {4:.3f}  {5:.3f} {6:.3f}   {7:.3f}'.format(
                epoch, train_loss, test_loss, train_train_loss, test_train_loss,
        ...
        losses += [(
            ...
            train_train_loss, test_train_loss,
...
def _evaluate(sess, validation_steps, log_loss_op, train_log_loss_op, accuracy_op):

    test_loss, test_train_loss, test_accuracy = 0.0, 0.0, 0.0
    for i in range(validation_steps):
        batch_loss, batch_train_loss, batch_accuracy = sess.run(
            [log_loss_op, train_log_loss_op, accuracy_op], {'control/is_training:0': False}
        )
        test_loss += batch_loss
        test_train_loss += batch_train_loss
        test_accuracy += batch_accuracy

    test_loss /= validation_steps
    test_train_loss /= validation_steps
    test_accuracy /= validation_steps

    return test_loss, test_train_loss, test_accuracy
